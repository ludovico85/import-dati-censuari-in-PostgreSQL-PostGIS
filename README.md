...
```sql
SET search_path to catasto_fabbricati;
CREATE OR REPLACE VIEW fab_1_colnames AS
SELECT
field_1 AS codice_amministrativo,
field_2 AS sezione,
field_3 AS identificativo_immboile,
...
field_14 AS rendita_lotto,
field_15 AS edificio,
field_16 AS scala,
field_17 AS interno_1,
field_17 AS interno_2,
field_17 AS piano_1,
field_17 AS piano_2,
field_17 AS piano_3,
field_17 AS piano_4
...
```
```sql
CREATE OR REPLACE VIEW fab_3_colnames AS
SELECT
field_1 AS codice_amministrativo,
field_2 AS sezione,
field_3 AS identificativo_immboile,
...
CASE WHEN field_11 IS NULL AND field_10 IS NULL THEN concat (t.denominazione,' ',field_8, ' ', regexp_replace(field_9, '^0+', '')) ELSE
	(CASE WHEN field_11 IS NULL AND field_10 IS NOT NULL THEN concat (t.denominazione,' ',field_8, ' ', regexp_replace(field_9, '^0+', '')) ELSE
	 	(CASE WHEN field_11 IS NOT NULL AND field_10 IS NOT NULL THEN concat (t.denominazione,' ', field_8, ' ', regexp_replace(field_9, '^0+', ''))END)
	 END)
END AS indirizzo_completo,
CASE WHEN field_17 IS NULL AND field_16 IS NULL THEN concat (a.denominazione,' ',field_14, ' ', regexp_replace(field_15, '^0+', '')) ELSE
	(CASE WHEN field_17 IS NULL AND field_16 IS NOT NULL THEN concat (a.denominazione,' ',field_14, ' ', regexp_replace(field_15, '^0+', '')) ELSE
	 	(CASE WHEN field_17 IS NOT NULL AND field_16 IS NOT NULL THEN concat (a.denominazione,' ', field_14, ' ', regexp_replace(field_15, '^0+', ''))END)
	 END)
END AS indirizzo_completo_a,
CASE WHEN field_23 IS NULL AND field_22 IS NULL THEN concat (b.denominazione,' ',field_20, ' ', regexp_replace(field_21, '^0+', '')) ELSE
	(CASE WHEN field_23 IS NULL AND field_22 IS NOT NULL THEN concat (b.denominazione,' ',field_20, ' ', regexp_replace(field_21, '^0+', '')) ELSE
	 	(CASE WHEN field_23 IS NOT NULL AND field_22 IS NOT NULL THEN concat (b.denominazione,' ', field_20, ' ', regexp_replace(field_21, '^0+', ''))END)
	 END)
END AS indirizzo_completo_b,
CASE WHEN field_29 IS NULL AND field_28 IS NULL THEN concat (c.denominazione,' ',field_26, ' ', regexp_replace(field_27, '^0+', '')) ELSE
	(CASE WHEN field_29 IS NULL AND field_28 IS NOT NULL THEN concat (c.denominazione,' ',field_26, ' ', regexp_replace(field_27, '^0+', '')) ELSE
	 	(CASE WHEN field_29 IS NOT NULL AND field_28 IS NOT NULL THEN concat (c.denominazione,' ', field_26, ' ', regexp_replace(field_27, '^0+', ''))END)
	 END)
END AS indirizzo_completo_c
FROM fab_3
LEFT JOIN toponimo t ON CAST(fab_3.field_7 AS INTEGER) = t.codice_toponimo
LEFT JOIN toponimo a ON CAST(fab_3.field_13 AS INTEGER) = a.codice_toponimo
LEFT JOIN toponimo b ON CAST(fab_3.field_19 AS INTEGER) = b.codice_toponimo
LEFT JOIN toponimo c ON CAST(fab_3.field_25 AS INTEGER) = c.codice_toponimo
```
```sql
CREATE OR REPLACE VIEW fab_4_colnames AS
SELECT
field_1 AS codice_amministrativo,
field_2 AS sezione,
field_3 AS identificativo_immboile,
